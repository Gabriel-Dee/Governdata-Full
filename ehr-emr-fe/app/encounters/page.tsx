...
import { listEncounters } from "@/lib/api/endpoints"
import type { EncounterDto } from "@/lib/api/types"
import { ApiError } from "@/lib/api/client"
import { CalendarDays, UserRound, FolderSearch, ChevronLeft, ChevronRight } from "lucide-react"
...
export default function EncountersPage() {
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      try {
        setIsLoading(true)
        setError(null)
        const page = await listEncounters({ page: pageNumber, size: pageSize })
        if (!isMounted) return
  ...
  return (
    <SecuredPage requiredAny={["encounter.read"]}>
      {isLoading ? (
        <TablePageSkeleton />
      ) : (
        <>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-2xl lg:text-3xl font-semibold text-[#1F1F1F]">Encounters</h1>
              <p className="text-[#989898] text-sm mt-1">Global list from `/api/v1/encounters`</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-5 mb-5">
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-5 border border-white/30">
              <div className="flex items-center justify-between">
                <p className="text-xs text-[#989898]">Total encounters</p>
          ...
          <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-5 border border-white/30 overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {encounters.map((encounter) => (
                  <tr key={encounter.id} className="border-b border-gray-50 text-[#1F1F1F] hover:bg-white/40 transition-colors">
                    <td className="py-3 pr-4">{encounter.patientId}</td>
                    <td className="py-3 pr-4">{new Date(encounter.encounterDate).toLocaleString()}</td>
                    <td className="py-3 pr-4">{encounter.type}</td>
                    <td className="py-3 pr-4">{encounter.reason}</td>
                    <td className="py-3 pr-4">{encounter.providerId}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            {encounters.length === 0 ? (
              <div className="py-10 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm font-medium text-[#1F1F1F]">No encounters found</p>
                <p className="text-xs text-[#989898] mt-1">No encounter rows returned on this page.</p>
              </div>
            ) : null}
          </div>
